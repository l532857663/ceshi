...
def logging_in():
	...
	#输出到文件
	fh = logging.FileHandler("log_get_txt1_data.log")
...
def analysis_txt_data(the_data):
	...
	try:
		...
		if len(the_data) == 2:
			...
			tmp_data["username"] = the_data[1].decode("utf-8", "ignore")
			tmp_data["password"] = the_data[2].decode("utf-8", "ignore")
			tmp_data["email"] = the_data[1].decode("utf-8")
			txt_data_dict["name"] = the_data[0].decode("gbk")
		else:
			return
		txt_data_dict["index"] = "olddata"
		txt_data_dict["type"] = "nccu.edu.tw"
...
def txt_analysis(the_file):
	try:
		...
		flage = False
		patter_bytes = bytes(r'<tw_personal_info_list>(.*)<tw_personal_info_list>,', encoding = "utf-8")
		with open(the_file, "rb") as f:
			line = f.readline()
			while line:
				line_data = line.strip(b"\r\n")
				if not flage:
					res = re.findall(patter_bytes, line_data, re.I)
					if len(res) == 0:
						logger.warning("the file not have title")
					print(res)
					flage = True
				line = f.readline()
		exit()
...
def main():
	...
	txt_analysis("/home/w123/files/olddata2/tw-shop/19286.txt")
